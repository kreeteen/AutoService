...
public class AutoServiceFacade {
    ...
    // В класс AutoServiceFacade добавьте:
    public List<OrderEmployee> getOrderEmployeesForAll() {
        // Получаем все заказы и собираем все назначения
        return getAllOrders().stream()
                .flatMap(order -> getOrderEmployees(order.getOrderId()).stream())
                .toList();
    }
